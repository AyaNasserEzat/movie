...
void main() {
  ...
  test("returns Right(List<MovieEntity>) when remoteDataSource succeeds", () async {
    // رجع قائمة من MovieModel
    when(remoteDataSource.getNowPlaying()).thenAnswer((_) async => movieModel);
...
final failure=Failure(errorMessage: "Server Error");
  test("returns Left(Failure) when remoteDataSource throws ServerException", () async {
  
    when(remoteDataSource.getNowPlaying()).thenThrow(
 ServerException(
    ErrorModel(status: 404, errorMessage: "Server Error"),
    ),
